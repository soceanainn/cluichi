...
if [[ $1  == '' || $2 == '' || $3 == '' ]]; then
    echo "Expected '$0 <files/downloaded_file_name.tsv> <title> <page_name>'"
    exit 1
fi
...
python3 files/generate_page_utils.py $1 $3

BACKGROUND_IMAGE=""
if [[ -f "$3/background.jpg" ]]; then
    BACKGROUND_IMAGE="background: url('background.jpg') no-repeat center center fixed;"
elif [[ -f "$3/background.png" ]]; then
    BACKGROUND_IMAGE="background: url('background.png') no-repeat center center fixed;"
fi

cat << EOF > $3/index.html
<!DOCTYPE html>
<html lang="ie">
<head>
   <title>$2</title>
